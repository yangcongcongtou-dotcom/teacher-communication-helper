...
function renderResults(results) {
  ...
  $("#results").innerHTML = labels.map(([key, label]) => `
    <article class="result-card ${key === "advice" ? "advice-card" : ""}" data-result-key="${key}">
      <h2>${label}</h2>
...
function recoverLastGenerationFromVisibleResults() {
  const resultCards = $$("[data-result-key]");
  if (!resultCards.length) return false;

  const results = {};
  resultCards.forEach((card) => {
    const key = card.dataset.resultKey;
    const body = $(".result-body", card);
    if (key && body) {
      results[key] = body.textContent.trim();
    }
  });

  if (!results.brief || !results.warm || !results.formal) {
    return false;
  }
...
async function handleAIOptimize() {
  if (!state.lastGeneration && !recoverLastGenerationFromVisibleResults()) {
